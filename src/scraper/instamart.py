...
Requires Swiggy session — Selenium gives us the best chance.
"""
...
class InstamartScraper(BaseScraper):
    ...
    delivery_mins = 18   # Swiggy Instamart ~18 min

    def _fetch(self, query: str, driver: webdriver.Chrome) -> List[Dict]:
        url  = SEARCH_URL.format(query=query.replace(" ", "+"))
        soup = self._get_page(driver, url,
                              wait_selector="[class*='product'],[class*='Product'],[data-testid]",
                              timeout=22)

        page_src = driver.page_source.lower()
        if any(k in page_src for k in ["login", "sign in", "set location",
                                        "allow location", "select city"]):
            self.log("  [Instamart] ⚠️  Login/location wall detected")

        # Try __NEXT_DATA__ / React hydration JSON
    ...
    def _walk(self, nd) -> List[Dict]:
        ...
        for item in max(cands, key=len)[:40]:
            ...
            price = (item.get("price") or item.get("sp") or
                     item.get("selling_price") or item.get("mrp"))
    ...
    def _dig(self, obj, found, depth):
        ...
        if isinstance(obj, list) and obj and isinstance(obj[0], dict):
            if set(obj[0]) & {"price","sp","name","product_name","display_name"}:
